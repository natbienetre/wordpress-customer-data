...
class Tool_Inspect_Token {
	 ...
	 * @var string
	 */
	public const SCRIPT_HANDLE = 'vfs-tool-inspect-token';
	 ...
	 * @var string
	 */
	public const INTERACTIVITY_STORE_NAMESPACE = 'vfs-admin-tools-token-inspection';
	 ...
	 * @var string
	 */
	public const TOKEN_INSPECTION_INPUT_NAME = 'vfs-token-inspection-token';
	 ...
	 * @var string
	 */
	public const NONCE_NAME = 'vfs-action-inspect-token';
	...
	public function render() {
		...
		?>
		<div id="vfs-token-inspection-container" class="card"
			...
			<h2 class="title"><?php esc_html_e( 'Token inspection', 'vfs' ); ?></h2>
			<form id="vfs-token-inspection-form" method="get">
				...
				<p>
					<input
						id="vfs-token-inspection-token"
						type="text"
						name="<?php echo esc_attr( self::TOKEN_INSPECTION_INPUT_NAME ); ?>"
						class="regular-text"
						value="<?php echo esc_attr( $user_value ); ?>"
						placeholder="<?php esc_attr_e( 'xxxxxheaderxxxxx.yyyyypayloadyyyyyy.zzzzzsignaturezzzzz', 'vfs' ); ?>"
						data-wp-on--input="callbacks.inspectToken"
						data-wp-on--change="callbacks.pushHistory"
					/>
					<?php submit_button( __( 'Inspect', 'vfs' ), 'primary', 'submit', false ); ?>
				</p>
			</form>
			<div id="vfs-token-inspection-result" class="vfs-token-inspection-result" data-wp-bind--hidden="!context.token" data-wp-class--vfs-token-inspection-result-empty="!context.token" <?php echo $token_data ? '' : 'hidden=""'; ?>>
				<div class="vfs-token-inspection-result-notices" data-wp-bind--hidden="!context.token.validity.notices.length">
				...
				<div data-wp-bind--hidden="!context.token.payload" <?php echo empty( $token_data['payload'] ) ? '' : 'hidden=""'; ?> class="notice-success notice inline">
					<div class="vfs-token-inspection-result-metadata">
						<?php esc_html_e( 'Version', 'vfs' ); ?>
						<span
							class="vfs-token-inspection-result-version"
							data-wp-text="context.token.payload.version"
						><?php echo esc_html( $token_data['payload']['version'] ?? '' ); ?></span>
						<span
							class="vfs-token-inspection-result-algo"
							data-wp-text="context.token.metadata.algorithm"
						><?php echo esc_html( $token_data['metadata']['algorithm'] ?? '' ); ?></span>
					</div>
					<div class="vfs-token-inspection-result-user"
						title="<?php echo esc_attr( $token_data['payload']['user']['id'] ?? '' ); ?>"
						data-wp-bind--title="context.token.payload.user.id"
					>
						<span data-wp-text="context.token.payload.user.displayName" class="vfs-token-inspection-result-user-name"><?php echo esc_html( $token_data['payload']['user']['displayName'] ?? '' ); ?></span>
						<span data-wp-text="context.token.payload.user.email" class="vfs-token-inspection-result-user-email"><?php echo esc_html( $token_data['payload']['user']['email'] ?? '' ); ?></span>
					</div>
					<div class="vfs-token-inspection-result-expiry"><?php esc_html_e( 'Expires at ', 'vfs' ); ?>
						<span
							class="vfs-token-inspection-result-expiry-date <?php echo ( $token_data['payload']['swift']['expiresAt'] ?? 0 ) > time() ? '' : 'expired'; ?>"
					...
					<div class="vfs-token-inspection-result-scope"><?php esc_html_e( 'Scope:', 'vfs' ); ?>
						...
						<i data-wp-bind--hidden="context.token.payload.swift.pageSpace" <?php echo empty( $token_data['payload']['swift']['pageSpace'] ) ? 'hidden=""' : ''; ?>><?php esc_html_e( 'Unscoped', 'vfs' ); ?></i>
					</div>
					<div class="vfs-token-inspection-result-method"><?php esc_html_e( 'Methods:', 'vfs' ); ?>
						<?php
						foreach ( Swift_Api::all_http_methods() as $method ) :
							?>
							<span class="vfs-token-inspection-result-method-item <?php echo empty( $token_data['payload']['swift']['signatures'][ $method ] ) ? 'vfs-denied-method' : 'vfs-allowed-method'; ?>" data-wp-class--vfs-allowed-method="context.token.payload.swift.signatures.<?php echo esc_js( $method ); ?>" data-wp-class--vfs-denied-method="!context.token.payload.swift.signatures.<?php echo esc_js( $method ); ?>">
	...
	public function token_data( $token ) {
		...
		try {
			...
			return array(
				...
				'validity' => array(
					...
					'notices' => array( sprintf( __( 'Invalid token: %s', 'vfs' ), esc_html( $e->getMessage() ) ) ),
				),
			);
		}
		...
		if ( ! $token_type->supports( $jws ) ) {
			$results->add(
				...
				__( 'Token type not supported', 'vfs' ),
				array(
					'token' => $jws,
				)
			);
